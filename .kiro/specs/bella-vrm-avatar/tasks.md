...
- [ ] 7. Auto-blink state machine
  - [ ] 7.1 Implement three-state blink machine in rAF loop
  ...
  - [ ]* 7.3 Write property test for blink state transitions (Property 3)
...
- [ ] 8. Lip sync
  - [ ] 8.1 Implement lip sync toggle in rAF loop
  ...
  - [ ]* 8.2 Write property test for lip sync Aa range (Property 5)
...
- [ ] 9. Emotion expressions
  - [ ] 9.1 Implement `useEffect` on `emotion` prop in `VRMViewer`
  ...
  - [ ] 9.2 Write property test for emotion expression mapping (Property 4)
...
- [ ] 10. BellaOverlay state management and handlers
  - [ ] 10.1 Add `isRecording` state and `sessionId` ref to `BellaOverlay`
  ...
  - [ ] 10.2 Implement `playTTS(text: string)` handler
  ...
  - [ ] 10.3 Implement `handleSend()` wired to real backend
  ...
  - [ ] 10.4 Implement `addBellaMessage(text, emotion)` calling `playTTS`
    - Append bella message to thread, call `playTTS(text)`, set emotion
    - _Requirements: 8.1, 6.3_
  - [ ] 10.5 Implement `handleMicToggle()` with `MediaRecorder`
  ...
  - [ ] 10.6 Write property test for TTS fallback duration clamping (Property 9)
  ...
  - [ ] 10.7 Write property test for message history append-only (Property 8)
...
- [ ] 11. ChatPanel UI
  - [ ] 11.1 Extract `ChatPanel` as inline component within `BellaOverlay.tsx`
  ...
  - [ ] 11.2 Add mic button to ChatPanel input row
  ...
  - [ ] 11.3 Write property test for send button disabled logic (Property 6)
  ...
  - [ ] 11.4 Write property test for message alignment (Property 7)
...
- [ ] 12. Loading shimmer state
  - [ ] 12.1 Ensure shimmer renders while `!vrmLoaded` and hides on `onLoaded` callback
...
- [ ] 15. Backend property-based tests
  - [ ] 15.1 Write property test for chat history round-trip order (Property 10)
  ...
  - [ ] 15.2 Write property test for emotion expression mapping backend-side (Property 4)
    - **Property 4: Emotion state maps to correct expression values**
    - **Validates: Requirements 5.1, 5.2, 5.3**
...
- [ ] 16. Frontend unit tests
  - [ ] 16.1 Write `frontend/components/bella/BellaOverlay.test.tsx`

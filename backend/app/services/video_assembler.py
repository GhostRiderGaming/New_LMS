...
def _create_title_card(title: str, subtitle: str = "", duration: float = _TITLE_CARD_DURATION) -> tuple[ImageClip, str]:
    """Create a cinematic title card with gradient background.
    
    Returns (clip, temp_path) so the caller can clean up the temp file.
    """
...
def _create_scene_clip(image_bytes: bytes, caption: str, duration: float) -> tuple[ImageClip, str]:
...
async def assemble_story_video(
    ...
    try:
        ...
        opening, opening_tmp = _create_title_card(title, f"A {topic} Adventure")
        clips.append(opening)
        temp_files.append(opening_tmp)
        ...
        for ep in story_plan.get("episodes", []):
            ...
            # Episode title card
            ep_card, ep_tmp = _create_title_card(
                f"Episode {ep_num}: {ep_title}",
                concept,
                duration=3
            )
            clips.append(ep_card)
            temp_files.append(ep_tmp)
            ...
            for scene in ep.get("scenes", []):
                ...
                scene_clip, scene_tmp = _create_scene_clip(img_bytes, caption, duration)
                temp_files.append(scene_tmp)
        ...
        # 3. Closing card
        closing, closing_tmp = _create_title_card("The End", f"Generated by AnimeEdu • {topic}", duration=3)
        clips.append(closing)
        temp_files.append(closing_tmp)
